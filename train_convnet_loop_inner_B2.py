...
def main(rank, world_size, causality_awareness, learning_rate, weight_decay, causality_method=None, lehmer_param=None, causality_setting="cat",mulcat_causes_or_effects="causes"):
    ...
    # results_folder = Path("./results_june") #esperimenti sistematici giugno luglio e primi di agosto

    # results_folder = Path("./results_september") #TODO
    results_folder = Path("./results_september_DEBUG") #TODO
    ...
    #     print(f"IMAGES[0] (trainloader):\t MIN:{images[0].min()}\t MAX:{images[0].max()}\t AVG:{torch.mean(images[0].float())}\t MED:{torch.median(images[0].float())}")
    ...
    if model_type=="LightCNN":
        ...
        from networks_attn import Resnet18CA #TODO 12 july 2023
        model = Resnet18CA(
            dim=image_size,
            channels=channels,
            num_classes=num_classes,
    ...
    scheduler = lr_scheduler.LinearLR(optimizer, start_factor=1.0, end_factor=1.0, total_iters=epochs) #TODO annullato il suo effetto
    ...
    # TODO 14 settembre 2023: creata una altra temporanea
    model_type_TMP = model_type+which_resnext_to_use

    if causality_awareness:
        if causality_method=="lehmer":
            if model_type=="EqualCNN":
                path_to_model_dir = os.path.join(results_folder,"saved_models",CONDITIONING_FEATURE, model_type_TMP, causality_setting_TMP, str(number_of_feature_maps), f"CA_{causality_awareness}", causality_method, str(lehmer_param), f"{save_stamp}")
            else:
                path_to_model_dir = os.path.join(results_folder,"saved_models",CONDITIONING_FEATURE, model_type_TMP, causality_setting_TMP, f"CA_{causality_awareness}", causality_method, str(lehmer_param), f"{save_stamp}")

        else:
            if model_type=="EqualCNN":
                path_to_model_dir = os.path.join(results_folder,"saved_models",CONDITIONING_FEATURE, model_type_TMP, causality_setting_TMP, str(number_of_feature_maps), f"CA_{causality_awareness}", causality_method, f"{save_stamp}")
            else:
                path_to_model_dir = os.path.join(results_folder,"saved_models",CONDITIONING_FEATURE, model_type_TMP, causality_setting_TMP, f"CA_{causality_awareness}", causality_method, f"{save_stamp}")

    else:
        if model_type=="EqualCNN":
            path_to_model_dir = os.path.join(results_folder,"saved_models",CONDITIONING_FEATURE, model_type_TMP, str(number_of_feature_maps), f"CA_{causality_awareness}",f"{save_stamp}")
        else:
            path_to_model_dir = os.path.join(results_folder,"saved_models",CONDITIONING_FEATURE, model_type_TMP, f"CA_{causality_awareness}",f"{save_stamp}")
    ...
    def validation_loop(model, dataloader_valid, IS_CAUSALITY_AWARE, loss_function):
        ...
        if dist.get_rank()==0:
            if model_type=="EqualCNN":
                ...
                # handle_b = model.module.features.conv1.register_forward_hook(get_activation("conv1"))
                # handle_b = model.module.layer1.register_forward_hook(get_activation("layer1")) #TODO 6 sett 2023
                handle_b = model.module.layer3.register_forward_hook(get_activation("layer3")) #TODO 6 sett 2023
                    ...
                    if need_to_savefig and dist.get_rank()==0: #save the figure only if it is the MASTER process (GPU with rank 0)
                        ...
                        if model_type=="EqualCNN":
                            ...
                            for b_i in range(batch_causality_maps.size()[0]):
                                ...
                                fig=plt.figure(figsize=(20,20)) #2000px X 2000px = 1.6-2MB PNG #TODO 14 settembre 2023
                                # fig=plt.figure()
                                
                                ax = fig.add_subplot(111)
                                sns.heatmap(c_map[:16,:16], annot=True, annot_kws={"fontsize":6}, linewidths=.5, vmax=100, vmin=0, square=True)  #TODO 14 settembre 2023
                                # sns.heatmap(c_map, linewidths=.5, vmax=100, vmin=0, square=True)                            
                                ##
                                title = ax.set_title("\n".join(wrap(f"Ep{epoch} causality map of {b_i} valid img. Values from {c_map_min} to {c_map_max}. Elems over {c_map_threshold}%: {indices}", 200)))
                                fig.tight_layout()
                                title.set_y(1.05)
                            ...
                            # plt.savefig(os.path.join(path_to_feature_maps,f"ep{epoch}_lyr1.pdf"))
                            imshow(make_grid(feature_maps_hooked['layer3']))
                            plt.savefig(os.path.join(path_to_feature_maps,f"ep{epoch}_lyr3.pdf"))
                            plt.close()
                            handle_b.remove()
                            #
                            #####
                            for b_i in range(batch_causality_maps.size()[0]):
                                ...
                                ax = fig.add_subplot(111)
                                # sns.heatmap(c_map, annot=True, annot_kws={"fontsize":6}, linewidths=.5, vmax=100, vmin=0, square=True)  #TODO 14 settembre 2023
                                sns.heatmap(c_map[:32,:32], linewidths=1, vmax=100, vmin=0, square=True)                            
                                ##
                                # title = ax.set_title("\n".join(wrap(f"Ep{epoch} causality map of {b_i} valid img. Values from {c_map_min} to {c_map_max}. Elems over {c_map_threshold}%: {indices}", 200)))
                                fig.tight_layout()
                                # title.set_y(1.05)
                                ...
                                #plt.title(f"Ep{epoch} causality map of im{b_i} of valid batch\nvalues from {np.round(c_map.min(),decimals=1)} to {np.round(c_map.max(),decimals=1)} ({c_map_maxidx})\nindices elems over {c_map_threshold}: {indices}")
                                plt.show()
                                plt.savefig(os.path.join(path_to_causality_maps,f"ep{epoch}_cmap{b_i}.png"))
                                plt.close()
    ...
    for epoch in range(epochs):
        ...
        IS_CHECKPOINT = epoch % 150 == 0 # SAVE MODEL EVERY 10 EPOCHS, AS A CHECKPOINT DISREGARDING THE VALUE OF THE VALIDATION LOSS
        model.train()
